...
## $(date +%Y-%m-%d) - Prevent Nested Button Accessibility Regressions in Clickable Containers
**Learning:** When converting non-semantic clickable list items or containers (like an `<a>` without an `href`) into accessible elements, naive replacement with `<button>` can lead to critical HTML5 violations if the container houses child buttons (e.g., Edit/Delete actions). Furthermore, implementing a `div role="button"` alternative requires an `onKeyDown` handler that correctly filters out bubbled events from inner interactive elements to prevent functional regressions where keyboard activation of child buttons unintentionally triggers the parent action.
**Action:** When making containers accessible, always inspect children for interactive elements. If present, use `<div role="button" tabIndex={0}>` instead of `<button>`, and implement keyboard handlers with `if (e.target !== e.currentTarget) return;` to preserve independent functionality for nested interactive elements.

...
class CRUDAnalytics:
    ...
    def _calculate_sharpe_ratio(self, db: Session, portfolio_id: uuid.UUID) -> float:
        ...
        logger.debug(f"Sharpe Ratio Debug: Portfolio {portfolio_id} has {len(history_points)} history points.")
        ...
        logger.debug(f"Sharpe Ratio Debug: First value: {daily_values[0]}, Last value: {daily_values[-1]}")
        ...
        if len(daily_returns) == 0 or np.std(daily_returns) == 0:
            logger.debug(f"Sharpe ratio: Not enough returns or zero standard deviation. Returns count: {len(daily_returns)}, Std Dev: {np.std(daily_returns) if len(daily_returns)>0 else 'N/A'}")
            return 0.0

        sharpe_ratio = (np.mean(daily_returns) / np.std(daily_returns)) * np.sqrt(252)
        logger.debug(f"Sharpe ratio: Calculated value is {sharpe_ratio}. Mean return: {np.mean(daily_returns)}")

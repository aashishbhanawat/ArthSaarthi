...
def _get_standalone_asset_current_value(db: Session, asset_id: uuid.UUID, user_id: uuid.UUID) -> Decimal:
    ...
    current_prices_details = crud_analytics.financial_data_service.get_current_prices(asset_to_price)
    ...
    def get_goal_analytics(self, db: Session, goal: Goal) -> Dict[str, Any]:
        ...
        for link in goal.links:
            if link.portfolio_id:
                current_value += crud_analytics._get_portfolio_current_value(db, link.portfolio_id)
            elif link.asset_id:
                current_value += _get_standalone_asset_current_value(db, link.asset_id, goal.user_id)

...
def fix_rsu_links():
    ...
    try:
        ...
        for vest in rsu_vests:
            ...
            # Filter in python for date match (easier than sqlalchemy cast across DB types)
            candidate_sells = [
                s for s in sells 
            ...
            for sell in candidate_sells:
                logger.info(f"Checking Sell {sell.id} (Qty: {sell.quantity}) on {vest_date} against RSU Vest {vest.id} (Qty: {vest.quantity})")

...
def seed_transactions(db: Session) -> None:
    ...
    if not portfolio:
        ...
        portfolio_in = schemas.PortfolioCreate(
            name=portfolio_name, description="Portfolio with auto-seeded sample data."
        )
    ...
    for i in range(transactions_to_create):
        ...
        if is_buy:
            ...
            try:
                ...
                # This will likely fail if holdings are insufficient.
                # We can just log it and move on.
                logger.info(f"Skipping SELL for {ticker} due to insufficient holdings.")

    db.commit()  # Final commit for all created transactions
    logger.info(
        "Successfully created %s transactions for portfolio ID %s.",

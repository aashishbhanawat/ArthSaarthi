...
def create_backup(db: Session, user_id: uuid.UUID) -> Dict[str, Any]:
        ...
        for aid in transaction_assets:
            ...
            if a:
                if a.asset_type == "PPF":
                    ...
                    # Access related bond - handling both relationship styles just in case
                    b = a.bond
    ...
    # Fetch FDs
    fds = db.query(models.FixedDeposit).filter(models.FixedDeposit.user_id == user_id).all()
    fd_list = []
    for fd in fds:
        ...
        fd_list.append({
            ...
            "compounding_frequency": fd.compounding_frequency.upper() if fd.compounding_frequency else "ANNUALLY",
            "payout_type": fd.interest_payout.upper() if fd.interest_payout else "CUMULATIVE"
        })

    # Fetch RDs
    rds = db.query(models.RecurringDeposit).filter(models.RecurringDeposit.user_id == user_id).all()
    rd_list = []
    for rd in rds:
    ...
    # Portfolios list
    portfolio_list = [{"name": p.name, "description": p.description} for p in portfolios]
...
def restore_backup(db: Session, user_id: uuid.UUID, backup_data: Dict[str, Any]):
    ...
    try:
        with db.begin_nested():
            ...
            # Delete RDs
            db.query(models.RecurringDeposit).filter(models.RecurringDeposit.user_id == user_id).delete()
            ...
            for bond_data in data.get("bonds", []):
                ...
                # Models don't always have ISIN unique constraint enforced in get_by, but Asset model has unique index on ISIN.
                asset = db.query(models.Asset).filter(models.Asset.isin == isin).first()
                if not asset:
                    # Create Asset
                    asset_in = schemas.AssetCreate(
                        name=bond_data["name"],
                        ticker_symbol=isin, # Bonds often use ISIN as ticker or we generate one?
                        # Requirement doesn't specify ticker for bonds in backup.
                        # We can assume ticker=isin or check if backup provides ticker?
                        # Backup example for bonds array doesn't have ticker.
                        # Backup example for transaction (BUY) has ticker.
                        # So we should match.
                        # If bond_data doesn't have ticker, we might have issue.
                        # But wait, transaction with ISIN should find this asset.
                        asset_type="BOND",
                        currency="INR",
                        isin=isin
                    )
                    # Need unique ticker. If Bond definition doesn't have ticker, use ISIN.
            ...
            for fd_data in data.get("fixed_deposits", []):
                ...
                if p_name and p_name in portfolio_map:
                    ...
                    crud.fixed_deposit.create_with_owner(db, obj_in=fd_in, user_id=user_id)
            ...
            for rd_data in data.get("recurring_deposits", []):
                ...
                if p_name and p_name in portfolio_map:
                    ...
                    crud.recurring_deposit.create_with_owner(db, obj_in=rd_in, user_id=user_id)
            ...
            for tx_data in data.get("transactions", []):
                ...
                if "ppf_account_number" in tx_data:
                    ...
                    asset = db.query(models.Asset).filter(models.Asset.isin == tx_data["isin"]).first()
                    if not asset and "ticker_symbol" in tx_data:
                        asset = crud.asset.get_or_create_by_ticker(db, ticker_symbol=tx_data["ticker_symbol"])
                ...
                crud.transaction.create_with_portfolio(db, obj_in=tx_in, portfolio_id=p_id)
                ...
                for p_name in g_data.get("linked_portfolios", []):
                    if p_name in portfolio_map:
                        crud.goal_link.create_with_owner(
                            db,
                            obj_in=schemas.GoalLinkCreate(goal_id=goal.id, portfolio_id=portfolio_map[p_name]),
                            user_id=user_id
                        )

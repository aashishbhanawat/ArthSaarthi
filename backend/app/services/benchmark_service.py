...
class BenchmarkService:
    ...
    def _generate_synthetic_transactions(self, portfolio_id: str) -> list:
        ...
        for fd in fds:
            ...
            synthetic_txns.append(
                self._create_mock_txn(fd.start_date, fd.principal_amount, "BUY")
            )
            ...
            if (fd.interest_payout or "").upper() == "PAYOUT":
                ...
                interval = interval_map.get((fd.compounding_frequency or "").upper())
                if interval:
                    ...
                    while curr_date <= fd.maturity_date:
                        synthetic_txns.append(
                            self._create_mock_txn(curr_date, period_payout, "DIVIDEND")
                        )
                        curr_date += interval
            ...
            if fd.maturity_date <= date.today():
                ...
                synthetic_txns.append(
                    self._create_mock_txn(fd.maturity_date, maturity_value, "SELL")
                )
        ...
        for rd in rds:
            ...
            for _ in range(rd.tenure_months):
                if curr_date > date.today():
                    break
                synthetic_txns.append(
                    self._create_mock_txn(curr_date, rd.monthly_installment, "BUY")
                )
            ...
            if maturity_date <= date.today():
                ...
                synthetic_txns.append(
                    self._create_mock_txn(maturity_date, maturity_value, "SELL")
                )

        return synthetic_txns

    def _create_mock_txn(self, t_date: date, amount: Decimal, t_type: str) -> Any:
    ...
    def _calculate_risk_free_values(
        ...
        for d in date_range:
            ...
            if daily_flow_xirr != 0:
                xirr_cashflows.append((d_date, daily_flow_xirr))
        ...
        try:
            ...
            logger.warning(f"Failed to calculate risk-free XIRR: {e}")
            rf_xirr = 0.0
    ...
    def _calculate_category_benchmark(
        ...
        if txns:
            ...
            for txn in txns:
                ...
                a_type_norm = str(a_type).upper().replace("_", " ")
        ...
        try:
            ...
            for h in holdings_data.holdings:
                h_type = str(getattr(h, "asset_type", "")).upper().replace("_", " ")
                ...
                if h_type in equity_types:
                    ...
                    if h_type in ["FIXED DEPOSIT", "RECURRING DEPOSIT"]:
                        has_fd_or_rd = True
            logger.debug(
            ...
            logger.warning(f"Could not get holdings for category values: {e}")
        ...
        logger.debug(f"Category results keys: {list(results.keys())}")
    ...
    def calculate_benchmark_performance(
        ...
        if benchmark_mode == "category":
            ...
            category_data = self._calculate_category_benchmark(
            ...
            if base_result.get("portfolio_xirr", 0) == 0 and portfolio_id:
                pf_analytics = crud.analytics.get_portfolio_analytics(
                    self.db, portfolio_id=portfolio_id
                )
                if pf_analytics:
                    if isinstance(pf_analytics, dict):
                        base_result["portfolio_xirr"] = pf_analytics.get("xirr", 0.0)
                    else:
                        base_result["portfolio_xirr"] = getattr(
            ...
            logger.info(
                "Benchmark calculation (category) for portfolio %s took %.4f seconds.",
                portfolio_id,
        ...
        logger.info(
            "Benchmark calculation (%s) for portfolio %s took %.4f seconds.",
            benchmark_mode,
            portfolio_id,
            time.time() - start_time,
        )
    ...
    def _run_simulation(
        ...
        histories = {}
        if benchmark_mode == "hybrid" and hybrid_preset in HYBRID_PRESETS:
            preset = HYBRID_PRESETS[hybrid_preset]
            for comp in preset["components"]:
                ticker = comp["ticker"]
                hist = yf.get_index_history(ticker, start_date, end_date)
                if hist:
                    histories[ticker] = hist
                else:
                    logger.warning(
                        f"Could not fetch history for hybrid component {ticker}"
                    )
        else:
            hist = yf.get_index_history(benchmark_ticker, start_date, end_date)
            if hist:
                histories[benchmark_ticker] = hist
            else:
                logger.warning(
                    f"Could not fetch history for benchmark {benchmark_ticker}"
                )
        ...
        if portfolio_id:
            all_real_txns = crud.transaction.get_multi_by_portfolio(
                self.db, portfolio_id=portfolio_id
            )
            # If we used synthetic txns, we are by definition
            # doing a "full" portfolio benchmarking of what we have.
            is_full_portfolio = len(transactions) == (
                len(all_real_txns) + len(synthetic_txns)
            )
                ...
                "chart_data": [
                    {
                        ...
                        "benchmark_value": 0.0,
                        "invested_amount": 0.0,
                        "risk_free_value": rf_vals.get(d.date().isoformat(), 0.0),
                    }
                    for d in date_range
                ],
            }
        ...
        chart_data = self._simulate_daily(
            ...
            components=(
                ...
                if (benchmark_mode == "hybrid" and hybrid_preset in HYBRID_PRESETS)
                else [{"ticker": benchmark_ticker, "weight": 1.0}]
            ),
            risk_free_rate=risk_free_rate,
            rf_daily_values=rf_vals,
        )
    ...
    def _calc_subset_xirr(
        ...
        try:
            ...
            for txn in transactions:
                ...
                if tx_type in BUY_TYPES:
                    pf_cashflows.append((txn.transaction_date.date(), -amount))
    ...
    def _simulate_daily(
        ...
        chart_data = []
        bench_units = {comp["ticker"]: 0.0 for comp in components}
        ...
        for d in date_range:
            ...
            for comp in components:
                ...
                if ticker in histories:
                    daily_prices[ticker] = self._get_price_for_date(
                ...
                if tx_type in BUY_TYPES:
                    ...
                    for comp in components:
                        ...
                        if price > 0:
                            if ticker not in histories:
                                bench_units[ticker] += comp_amt
                            else:
                                bench_units[ticker] += comp_amt / price
                    ...
                    if total_val > 0:
                        ...
                        for comp in components:
                            ticker = comp["ticker"]
                            if ticker not in histories:
                                bench_units[ticker] -= bench_units[ticker] * ratio
                            else:
                                bench_units[ticker] -= bench_units[ticker] * ratio
            ...
            for comp in components:
                ...
                if ticker not in histories:
                    ...
                    bench_value += bench_units[ticker] * price if price > 0 else 0.0
    ...
    def _calc_bench_xirr(
        ...
        for d in date_range:
            ...
            if daily_flow != 0:
                xirr_cashflows.append((d.date(), daily_flow))
        ...
        try:
            ...
            bench_xirr = float(bench_xirr) if bench_xirr else 0.0
            logger.debug(f"Benchmark XIRR calculated: {bench_xirr}")
            return bench_xirr
        except Exception as e:

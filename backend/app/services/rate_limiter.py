...
class RateLimitExceededException(Exception):
    """Raised when an external data provider exceeds its configured rate limit."""
    ...
    def __init__(
        ...
        self.scope = scope
        msg = (
            f"Rate limit ({scope}) exceeded for provider '{provider_name}': "
            f"{limit} calls per {window_seconds}s"
        )
        super().__init__(msg)


# Default provider limits: (max_global_calls, window_seconds, max_user_calls)
...
class ProviderRateLimiter:
    ...
    def check_and_increment(
        ...
        """
        Checks if a call to provider_name is within shared global & user limits.
        ...
        if user_id:
            u_key = (
                f"ratelimit:user:{user_id}:{provider_key}:"
                f"{window_seconds}s:{window_bucket}"
            )
            if self.cache:
                try:
                    u_count = self.cache.incr(u_key, expire=window_seconds * 2)
                    if u_count > user_limit:
                        logger.warning(
                            f"Per-user rate limit exceeded for user '{user_id}' "
                            f"on provider '{provider_name}': {u_count}/{user_limit}"
                        )
                        raise RateLimitExceededException(
                            provider_name, user_limit, window_seconds, scope="per-user"
                        )
                ...
                prev_u_count, prev_u_time = self._local_user_counts.get(
                    user_local_key, (0, now)
                )
                if now - prev_u_time >= window_seconds:
                    ...
                    self._local_user_counts[user_local_key] = (
                        prev_u_count + 1,
                        prev_u_time,
                    )
        ...
        if self.cache:
            try:
                ...
                if g_count > global_limit:
                    logger.warning(
                        f"Global shared rate limit exceeded for provider "
                        f"'{provider_name}': {g_count}/{global_limit}"
                    )
                    raise RateLimitExceededException(
                        provider_name, global_limit, window_seconds, scope="global"
                    )
                return True
            except RateLimitExceededException:
                raise
        ...
        prev_g_count, prev_g_time = self._local_global_counts.get(
            provider_key, (0, now)
        )

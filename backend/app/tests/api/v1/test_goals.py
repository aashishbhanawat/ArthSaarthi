...
def test_read_goals(client: TestClient, db: Session, get_auth_headers):
    ...
    client.post(f"{settings.API_V1_STR}/goals/", headers=headers, json={"name": "Goal 1", "target_amount": 1000, "target_date": date.today().isoformat()})
    client.post(f"{settings.API_V1_STR}/goals/", headers=headers, json={"name": "Goal 2", "target_amount": 2000, "target_date": date.today().isoformat()})
...
def test_read_goal_by_id(client: TestClient, db: Session, get_auth_headers):
    ...
    create_response = client.post(f"{settings.API_V1_STR}/goals/", headers=headers, json={"name": "My Goal", "target_amount": 5000, "target_date": date.today().isoformat()})
    goal_id = create_response.json()["id"]

    response = client.get(
        f"{settings.API_V1_STR}/goals/{goal_id}", headers=headers
    )
    assert response.status_code == 200
    content = response.json()
...
def test_update_goal(client: TestClient, db: Session, get_auth_headers):
    ...
    create_response = client.post(f"{settings.API_V1_STR}/goals/", headers=headers, json={"name": "Old Name", "target_amount": 1000, "target_date": date.today().isoformat()})
    goal_id = create_response.json()["id"]
...
def test_delete_goal(client: TestClient, db: Session, get_auth_headers):
    ...
    create_response = client.post(f"{settings.API_V1_STR}/goals/", headers=headers, json={"name": "To Be Deleted", "target_amount": 100, "target_date": date.today().isoformat()})
    goal_id = create_response.json()["id"]
...
def test_create_goal_link_portfolio(client: TestClient, db: Session, get_auth_headers):
    ...
    goal_res = client.post(f"{settings.API_V1_STR}/goals/", headers=headers, json={"name": "Link Goal", "target_amount": 10000, "target_date": date.today().isoformat()})
    goal_id = goal_res.json()["id"]

    portfolio = create_test_portfolio(db, user_id=user.id)
...
def test_delete_goal_link(client: TestClient, db: Session, get_auth_headers):
    ...
    link_res = client.post(f"{settings.API_V1_STR}/goals/{goal_id}/links", headers=headers, json={"portfolio_id": str(portfolio.id)})

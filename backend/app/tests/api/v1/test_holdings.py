...
def test_get_portfolio_summary_and_holdings_success(
    ...
    create_test_transaction(db, portfolio_id=portfolio.id, ticker="ASSET1", transaction_type="BUY", quantity=10, price_per_unit=100)
    create_test_transaction(db, portfolio_id=portfolio.id, ticker="ASSET1", transaction_type="SELL", quantity=5, price_per_unit=120)

    # Asset 2: Buy 20 @ 50
    # Expected holding: 20 shares, avg_buy_price=50, total_invested=1000
    create_test_transaction(db, portfolio_id=portfolio.id, ticker="ASSET2", transaction_type="BUY", quantity=20, price_per_unit=50)

    # Mock the external financial data service
    mock_prices = {
        "ASSET1": {"current_price": Decimal("150.0"), "previous_close": Decimal("140.0")},

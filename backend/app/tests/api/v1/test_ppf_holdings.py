...
# This mark is essential for tests that interact with encrypted data in desktop mode.
...
def test_ppf_interest_calculation(
    ...
    # Verification
    # Interest is calculated for all *completed* financial years up to today's
    # date (mocked to 2025-09-13)
    # FY 2022-23 Interest: 710.00
    # FY 2023-24 Interest: 15710.00 * 7.1% = 1115.41
    # FY 2024-25 Interest: 16825.41 * 7.1% = 1194.60
    # Total Credited Interest (Realized PNL) = 710.00 + 1115.41 + 1194.60 = 3020.01 # noqa: E501
    ...
    expected_unrealized_pnl = Decimal("533.09")  # Interest for the current FY
    expected_current_value = (
        total_investment + expected_realized_pnl + expected_unrealized_pnl
    )
    ...
    print(
        ...
        f"{expected_current_value.quantize(Decimal('0.01'))}"
    )
    print(f"Actual Current Value:    {ppf_holding['current_value']}")
    print("-----------------------------\n")

    assert (
        Decimal(ppf_holding["total_invested_amount"]).quantize(Decimal("0.01"))
        == total_investment
    )
    assert (
        Decimal(ppf_holding["realized_pnl"]).quantize(Decimal("0.01"))
        == expected_realized_pnl
    )
    assert (
        Decimal(ppf_holding["unrealized_pnl"]).quantize(Decimal("0.01"))
        == expected_unrealized_pnl
    )
    assert Decimal(ppf_holding["current_value"]).quantize(
        Decimal("0.01")
    ) == expected_current_value.quantize(Decimal("0.01"))
    ...
    interest_txn = next(
        (
            ...
            and t.transaction_date.date() == date(2023, 3, 31)
        ),
        None,
    )
    assert interest_txn is not None
    assert interest_txn.quantity.quantize(Decimal("0.01")) == Decimal("710.00")
...
def test_ppf_smart_recalculation(
    ...
    # This should trigger the smart recalculation logic and delete the
    # interest credit.
    response = client.delete(
        f"{settings.API_V1_STR}/transactions/{contribution_id}?portfolio_id={portfolio.id}",
        headers=headers,
    )
    assert response.status_code == 200

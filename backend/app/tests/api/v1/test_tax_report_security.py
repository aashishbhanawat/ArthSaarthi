...
def test_capital_gains_requires_auth(client: TestClient) -> None:
    response = client.get(f"{settings.API_V1_STR}/capital-gains/?fy=2025-26")
    assert response.status_code == 401

def test_dividends_requires_auth(client: TestClient) -> None:
    response = client.get(f"{settings.API_V1_STR}/dividends/?fy=2025-26")
    assert response.status_code == 401

def test_capital_gains_cross_tenant_access_denied(
    client: TestClient,
    db: Session,
...
def test_dividends_cross_tenant_access_denied(
    client: TestClient,
    db: Session,
    get_auth_headers: Callable[[str, str], Dict[str, str]]
) -> None:

...
def test_calculate_benchmark_performance_empty(benchmark_service, mock_db):
    # Mock crud.transaction.get_by_portfolio to return empty list
    with pytest.patch("app.crud.transaction.get_by_portfolio", return_value=[]):
...
def test_calculate_benchmark_performance_success(benchmark_service, mock_db, mock_financial_service):
    ...
    with pytest.patch("app.crud.transaction.get_by_portfolio", return_value=[txn1, txn2]):
        # Mock Index History
        ...
        with pytest.patch("app.crud.analytics.get_portfolio_analytics", return_value=mock_pf_analytics):
            ...
            assert last_point["benchmark_value"] > 15000.0

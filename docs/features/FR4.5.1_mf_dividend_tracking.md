...
*   **`TransactionFormModal.tsx`:**
    ...
    *   **Updated Logic for Reinvestment:** If "Reinvested?" is checked, the form will also require the "NAV on Reinvestment Date". On submission, the frontend will create **two** transactions:
        1.  A `DIVIDEND` transaction for the total cash amount received.
        2.  A `BUY` transaction for the new units purchased, where `quantity = Total Dividend Amount / NAV`.
    *   This two-transaction approach ensures a complete and accurate audit trail, correctly updating both Realized P&L (from the dividend) and holdings (from the buy).
...
*   No changes are expected in the analytics engine, as it already handles `DIVIDEND` and `BUY` transactions correctly.

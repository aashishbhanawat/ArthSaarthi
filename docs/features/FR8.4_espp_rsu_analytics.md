...
*   **Foreign Currency Handling:** For foreign stocks (e.g., in USD), the `fmv_at_vest` is in the foreign currency. For all INR-based calculations (like P&L shown on the dashboard), this value must be multiplied by the `exchange_rate_to_inr` also stored in the transaction's metadata.
...
*   **Benefit Display:**
    *   `Total Discount Received = (Market Price - Purchase Price) * Quantity`
    *   This will be read from the transaction's `metadata` and displayed separately from the Unrealized P&L.
...
*   **Backend Unit Tests:**
    ...
    *   **Test RSU Unrealized P&L (INR Stock):**
        1.  Create a holding with an `RSU_VEST` transaction for an INR stock.
        2.  Mock the current price of the asset.
        3.  Call the holdings endpoint and assert that the `unrealized_pnl` is calculated as `(Current Price - FMV at Vest) * Quantity`.
    *   **Test RSU Unrealized P&L (Foreign Stock):**
        1.  Create a holding with an `RSU_VEST` for a USD stock, including `fmv_at_vest` and `exchange_rate_to_inr` in metadata.
        2.  Mock the current price (in USD) and a different current FX rate.
        3.  Call the holdings endpoint and assert that the `unrealized_pnl` is correctly calculated in INR using both the historical and current FX rates.

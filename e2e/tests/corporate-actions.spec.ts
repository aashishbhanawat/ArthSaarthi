...
test.describe.serial('Corporate Actions E2E Flow', () => {
    ...
    test('should correctly log a reinvested dividend and create a new BUY transaction', async ({ page }) => {
        ...
        await expect(updatedHoldingRow.getByRole('cell', { name: '10.4' }).first()).toBeVisible({ timeout: 10000 });
        await expect(updatedHoldingRow.getByRole('cell', { name: '₹100.96' })).toBeVisible();

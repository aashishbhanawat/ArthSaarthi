  ...
  return (
    <>
      <div className="md:hidden space-y-1">
        {rates.map((rate) => (
          <InterestRateCard key={rate.id} rate={rate} onEdit={onEdit} onDelete={onDelete} />
        ))}
      </div>
      ...
      <div className="hidden md:block overflow-x-auto">
        <table className="table-auto w-full">
          ...
          <tbody>
            {rates.map((rate) => (
              <tr key={rate.id} className="border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50">
                ...
                <td className="p-3">
                  <div className="flex justify-center items-center space-x-4">
                    <button onClick={() => onEdit(rate)} className="text-gray-500 hover:text-blue-600 dark:hover:text-blue-400" aria-label={`Edit rate for ${rate.scheme_name}`}>
                      <PencilSquareIcon className="h-5 w-5" />
                    </button>
                    <button onClick={() => onDelete(rate)} className="text-gray-500 hover:text-red-600 dark:hover:text-red-400" aria-label={`Delete rate for ${rate.scheme_name}`}>

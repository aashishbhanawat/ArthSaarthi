...
import { Goal, GoalCreate, GoalUpdate, GoalLink, GoalLinkCreateIn } from '../types/goal';
...
export const createGoalLink = async (goalId: string, data: GoalLinkCreateIn): Promise<GoalLink> => {
    const response = await apiClient.post<GoalLink>(`/api/v1/goals/${goalId}/links`, data);
    return response.data;
};

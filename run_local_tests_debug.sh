...
create_env_file() {
    echo "DEBUG: Starting create_env_file for DB_TYPE = $DB_TYPE"
...
EOF
        ...
        echo "DEBUG: Restarting postgresql"
...
run_migration_tests() {
    echo "DEBUG: Starting run_migration_tests"
    ...
    echo "DEBUG: Calling create_env_file"
    create_env_file 0 0
...
main() {
    ...
    echo "DEBUG: Calling check_prereqs"
    check_prereqs
    echo "DEBUG: Calling install_deps"
    install_deps

    echo "DEBUG: In case statement for TEST_SUITE = $TEST_SUITE"
    case $TEST_SUITE in
        all)
            run_lint
            run_frontend_tests

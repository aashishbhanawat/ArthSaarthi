...
echo "--- Starting macOS Intel (x64) Build ---"
...
# 6. Package Electron App for macOS Intel
echo "Step 6: Packaging Electron app for macOS Intel (x64)..."
# Build only for x64 to match the runner's architecture
# The backend binary is also x64, so only x64 Electron app will work properly
(cd frontend && npm run dist -- --mac --x64)

echo "--- macOS Intel (x64) Build Finished ---"
